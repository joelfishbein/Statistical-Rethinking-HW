...
globe_hw2 %>%
  summarise(`5.5%ile` = quantile(`Standardized Posterior`,  .055),
            `94.5%ile` = quantile(`Standardized Posterior`,  .945))
            
```

Again with tidybayes...

```{r}

tidybayes::median_qi(globe_hw2$`Standardized Posterior`, .width = .89)
...
This is the 50% span of the posterior with the greatest area under the curve.

```{r}

tidybayes::mode_hdi(globe_hw2$`Standardized Posterior`, .width = .89)


```


TODO: graph both of these and then see which is wider, and think about why....