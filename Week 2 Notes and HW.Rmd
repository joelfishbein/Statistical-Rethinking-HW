...
\$\$ \text{Weight}\_i = \text{Normal}(\mu\_i, \sigma) \\ \mu\_i = \alpha + \beta (\overline{\text{Height}} - \text{Height}\_i) \\ \alpha \\text{ \~ } \text{Normal}(60,10) \\ \beta \\text{ \~ } \text{Normal}(0,10) \\ \sigma \\text{ \~ } \text{Uniform}(0,10) \\
...
test_cases <-
  tibble(
    height = runif(100, 130, 170),
    height_mean_centered = height - mean(height),
    weight = rnorm(n = 100,
                  mean = (intercept + slope*(height_mean_centered))))
...
model1.fit <- brms::brm(weight ~ 1 + height_mean_centered, # intercept and height on the right side
      ...
      # iterations, warmpup = ?, info re running the model
      iter = 28000,
      warmup = 27000,
      cores = 4
          )
